...
void CommandInterpreter::cmd_dump_settings(const std::string& path) {
    ensure_connected();
    std::ofstream file(path);
    if (!file)
        throw std::runtime_error("cannot open '" + path + "' for writing");

    // Skip command/runtime registers: these are not persisted settings
    // (start/speed/demand) or are flash command channels, not configuration.
    auto is_excluded = [](uint16_t addr) {
        return addr == reg::kStartCommand || addr == reg::kCommandSpeed ||
               addr == reg::kCommandDemand || addr == reg::kAppFlashCommand ||
               addr == reg::kDriveFlashCommand;
    };

    file << "# COPRA fan settings clone - generated by 'dumpsettings'\n";
    file << "# Source fan: " << baud_ << " 8"
         << (parity_ == Parity::None ? "N" : parity_ == Parity::Even ? "E" : "O")
         << "1, address " << static_cast<int>(slave_) << "\n";
    file << "# Reload onto another fan with:  fan_tool -p <port> " << path
         << "\n";
    file << "# writeraw uses raw register values, so they reload exactly.\n";
    file << "# Note: comm-setting changes (baud/parity/address) only take\n";
    file << "#       effect after a power cycle.\n\n";

    int count = 0;
    for (const RegDef& r : copra_registers()) {
        if (r.space != RegSpace::Holding) continue;  // settings are holding regs
        if (is_excluded(r.address)) continue;
        uint16_t raw = master_.read_holding(r.address, 1)[0];
        file << "writeraw " << r.address << " " << raw << "   # " << r.name
             << " = " << decode_value(r, raw);
        if (!r.unit.empty()) file << " " << r.unit;
        file << "\n";
        ++count;
    }
...
CommandResult CommandInterpreter::execute(const std::string& raw_line) {
    ...
    try {
        ...
        } else if (cmd == "dumpsettings" || cmd == "savesettingsfile") {
            if (args.size() < 2)
                throw std::runtime_error("usage: dumpsettings <file>");
            cmd_dump_settings(args[1]);
        } else if (cmd == "program") {
